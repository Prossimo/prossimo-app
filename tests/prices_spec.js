...
test('Prices tests', function () {
    ...
    test('single unit tests', function () {
        ...
        let first_unit = current_project.units.first();
    ...
    test('single accessory tests', function () {
        ...
        let first_accessory = current_project.extras.first();
    ...
    test('subtotal project prices', function () {
        ...
        let total_prices = current_project.getTotalPrices();
        ...
        let subtotal_profit_units = _.reduce(current_project.units.map(function (unit) {
            return unit.getSubtotalProfit();
        }), function (memo, item) {
            return memo + item;
        }, 0);
        let subtotal_profit_extras = _.reduce(_.map(current_project.extras.getRegularItems(), function (unit) {
            return unit.getSubtotalProfit();
        }), function (memo, item) {
            return memo + item;
        }, 0);
        let hidden_cost = current_project.extras.getHiddenPrice();
