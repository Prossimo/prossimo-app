...
import {globalChannel} from '../../utils/radio';
    ...
    var PRICING_SCHEME_LINEAR_EQUATION = app.constants.PRICING_SCHEME_LINEAR_EQUATION;var UNIT_PROPERTIES = [
...
var MULLION_TYPES = [
    ...
    //  could know when we don't get the right thing, and try to fix thatfunction getDefaultFillingType(default_glazing_name, profile_id) {
        var dummy_type = {
            fillingType: 'glass',
            fillingName: 'Glass'
        };
...
const Unit = Backbone.Model.extend({
    ...
    defaults: function () {
        ...
        //  corresponding attribute (see example for root_section)getDefaultValue: function (name, type) {
            var default_value = '';
...
parse: function (data) {
            var unit_data = data && data.unit ? data.unit : data;var parsed_data = app.schema.parseAccordingToSchema(unit_data, this.schema);
        ...
        initialize: function (attributes, options) {
            ...
            if ( !this.options.proxy ) {
                ...
                this.setProfile();this.on('change:profile_id', function () {this.setProfile({ validate_filling_types: true });
                }, this);
                ...
                //  TODO: we want to do the same thing for filling typesthis.listenTo(globalChannel, 'validate_units:dictionaries', function () {
        ...
        //  TODO: this should be done at parse stepvalidateRootSection: function () {
    ...
    getDefaultUnitOptions: function () {
            ...
            App.settings.dictionaries.each(function (dictionary) {
                ...
                var is_optional = _.contains(rules, 'IS_OPTIONAL');
                    ...
                    if ( !is_optional && dictionary_id && profile_id ) {
                        //  TODO: we need to call this directly on `dictionary` iterablevar option = App.settings.dictionaries.getDefaultOrFirstAvailableOption(dictionary_id, profile_id);
        ...
        resetUnitOptionsToDefaults: function () {
            ...
            vardefault_options = this.getDefaultUnitOptions();
            if ( JSON.stringify( current_options .toJSON()) !== JSON.stringify(default_options.toJSON()) ) { this.get('unit_options').reset(default_options.models);
            }
},
        ...
        validateFillingTypes: function () {
            ...
            if ( invalid_flag ) {this.persist('glazing', getDefaultFillingType(undefined, this.profile.id).fillingName);
    ...
    getAreaInSquareMeters: function () {
        ...
        getSection: function (sectionId) {
            return Unit.findSection(this.generateFullRoot(), sectionId);
        },
    ...
    _updateSection: function (sectionId, func) {
        ...
        var sectionToUpdate = findSection(rootSection, sectionId);
    ...
    setCircular: function (sectionId, opts) {
        ...
        var section = findSection(root_section, sectionId);
        var update_data = {};
    ...
    setSectionSashType: function (sectionId, type) {
        ...
        var fullSection = findSection(full, sectionId);
    ...
    setSectionMullionPosition: function (id, pos) {
        ...
        removeSash: function (sectionId) {
            var glazing_name = this.get('glazing');
            var profile_id = this.profile && this.profile.id;this._updateSection(sectionId, function (section) {
    ...
    generateFullRoot: function (rootSection, openingParams) {
        ...
        if (rootSection.sections && rootSection.sections.length) {
            ...
            var isParentBottom = openingParams.isBottom;rootSection.sections = _.map(rootSection.sections, function (sectionData, i) {
                var sectionParams = {
                    x: null, y: null, width: null, height: null};
                ...
                // Set section data & glass sizessectionData.mullionEdges = _.clone(rootSection.mullionEdges);
                ...
                sectionData.parentId = rootSection.id;
                    ...
                    if (isLeft) {
                        ...
                        sectionParams.height = openingParams.height;sectionData.mullionEdges.left = rootSection.divider;
                    ...
                    sectionParams.width = openingParams.width;                    sectionParams.height = position - rootSection.openingParams.y -
                            mullionWidth / 2;
                        sectionData.mullionEdges.bottom = rootSection.divider;sectionData.thresholdEdge = false;
                    } else if (isBottom){
                        sectionParams.y = position + mullionWidth / 2;
                        sectionParams.width = openingParams.width;sectionParams.height = openingParams.height + openingParams.y - position -
                            mullionWidth / 2;
    ...
    canAddSashToSection: function (sectionId) {
        ...
        var section = findSection(fullRoot, sectionId);

        if (section.parentId === undefined) {
            return true;
        }

        var parentSection = findSection(fullRoot, section.parentId);
    ...
    getFixedAndOperableSectionsList: function (current_root) {
        ...
        getSectionsListWithEstimatedCost: function () {
            ...
            var sections_list = this.getFixedAndOperableSectionsList();var options_grouped_by_scheme = this.getUnitOptionsGroupedByPricingScheme();
            ...
            _.each(sections_list, function (section) {
                ...
                //  Add base cost for profileif ( profile_pricing_data && profile_pricing_data.scheme === PRICING_SCHEME_PRICING_GRIDS ) {
        ...
        getEstimatedUnitCost: function () {
            ...
            unit_cost.real_cost.difference = unit_cost.total ?
                ...
                (unit_cost.real_cost.total ? 100 : 0);return unit_cost;
        },
        ...
        var result = [];
            ...
            if ( App.settings ) {
                options_list.each( function (list_item) {
                    ...
                    vartarget_dictionary = App.settings.dictionaries.get(list_item.get('dictionary_id'));
    ...
    checkIfRestrictionApplies: function (restriction) {
        ...
        persistOption: function (dictionary_id, dictionary_entry_id, quantity) {
            ...
            var new_unit_options = new app.UnitOptionCollection(current_unit_options.filter( function (unit_option) {
    ...
    getTrapezoidHeights: function (inside) {
        ...
        var left = utils.convert.inches_to_mm(heights[0]);
        var right = utils.convert.inches_to_mm(heights[1]);
    ...
    checkHorizontalSplit: function (rootSection, params) {
        ...
        if (rootSection.divider && rootSection.divider === 'horizontal' && rootSection.position) {
            ...
            if (crossing >= -100) {
                var maxHeight = utils.convert.inches_to_mm(params.maxHeight);
                var minHeight = utils.convert.inches_to_mm(params.minHeight);
