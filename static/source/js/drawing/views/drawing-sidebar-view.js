...
(function () {
    ...
    app.DrawingSidebarView = Marionette.View.extend({
                ...
                active_unit_properties = _.map(relevant_properties, function (prop_name) {
                    return {
                        ...
                        value: function () {
                            ...
                            if (
                                active_unit instanceof app.Unit &&
                                active_unit.isOperableOnlyAttribute(prop_name) && !active_unit.hasOperableSections()
                            ) {
                                val = '(Operable Only)';
                            } else if (
                                active_unit instanceof app.Unit &&
                                active_unit.isGlazingBarProperty(prop_name) && !active_unit.hasGlazingBars()
                            ) {
                                val = '(Has no Bars)';
                            }
        ...
        getActiveUnitEstimatedCost: function () {
            ...
            if ( active_unit && active_unit instanceof app.Unit ) {
                active_unit_profile = active_unit.profile;
                var unit_cost = active_unit.getEstimatedUnitCost();
                ...
                result.original_currency = active_unit.get('original_currency');
                result.conversion_rate = f.fixed(active_unit.get('conversion_rate'), 3);
