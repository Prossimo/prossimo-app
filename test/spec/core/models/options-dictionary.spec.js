...
test('Options dictionary', () => {
    ...
    test('dictionary parse function', () => {
        const source_data = {
            name: 'Interior Handle',
        ...
        const broken_data = _.extend({}, source_data, { rules_and_restrictions: 'garbage' });

        const dictionary = new OptionsDictionary(source_data, { parse: true });
        const parsed_data = dictionary.parse(source_data);
        ...
        const dictionary_two = new OptionsDictionary(source_data, { parse: true });
        ...
        const dictionary_three = new OptionsDictionary(broken_data, { parse: true });
    ...
    test('dictionary.entries correctly calls validatePositions and validatePerProfileDefaults function', () => {
        const source_data = {
            name: 'Interior Handle',
            id: 22,
            entries: [
    ...
    test('dictionary toJSON function', () => {
        const dictionary = new OptionsDictionary(
            {
                name: 'Interior Handle',
                nonexistent_option: 'Whatever',
    ...
    test('dictionary.entries entrie_change event is triggered correctly', () => {
        const dictionary = new OptionsDictionary(
            {
                name: 'Interior Handle',
                entries: [
        ...
        const first_entry = dictionary.entries.at(0);

        let dictionary_event_counter = 0;
        let entry_event_counter = 0;

        dictionary.on('entries_change', () => {
            dictionary_event_counter += 1;
        });
        first_entry.on('change', () => {
