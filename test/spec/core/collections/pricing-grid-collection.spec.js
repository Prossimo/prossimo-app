...
test('Pricing grid collection', () => {
    test('basic tests', () => {
        const grids = new PricingGridCollection(null, { parse: true });

        equal(grids.length, 0, 'Grids contain 0 entries by default');

        const full_grids = new PricingGridCollection([
            {
                name: 'fixed',
                data: [
    ...
    test('pricing grid collection append_default_grids option tests', () => {
        const grids = new PricingGridCollection(null, { append_default_grids: true });
    ...
    test('pricing grid collection getByName function', () => {
        const grids = new PricingGridCollection([
            {
                name: 'fixed',
                data: [
                    { height: 500, width: 500, value: 55 },
        ...
        const fixed_grid = grids.getByName('fixed');
        const nonexistent_grid = grids.getByName('whatever');
    ...
    test('pricing grid collection json string parsing', () => {
        ...
        const collection_data_old_format = [
            {
                name: 'fixed',
                data: [
        ...
        const collection_data_old_format_2 = {
            fixed: [
                { title: 'Small', height: 500, width: 500, price_per_square_meter: 55 },
                { title: 'Medium', height: 914, width: 1514, price_per_square_meter: 50 },
                { title: 'Large', height: 2400, width: 3000, price_per_square_meter: 45 },
        ...
        const grids_2 = new PricingGridCollection(
    ...
    test('pricing grid collection toJSON function', () => {
        const collection_data = [
            {
                name: 'fixed',
                data: [
    ...
    test('pricing grid collection event propagation', () => {
        const collection_data = [
            {
                name: 'fixed',
                data: [
        ...
        const fixed_grid = grids.getByName('fixed');
        const fixed_grid_first_item = fixed_grid.get('data').at(0);
