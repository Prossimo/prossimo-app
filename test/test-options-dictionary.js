...
test('dictionary basic tests', function () {
    ...
    ok(dictionary.entries instanceof Backbone.Collection, 'Dictionary entries is properly set');
...
test('dictionary parse function', function () {
    ...
    var broken_data = _.extend({}, source_data, { rules_and_restrictions: 'garbage' });

    var dictionary = new app.OptionsDictionary(source_data, { parse: true });
    var parsed_data = dictionary.parse(source_data);
    ...
    var dictionary_two = new app.OptionsDictionary(source_data, { parse: true });
    ...
    var dictionary_three = new app.OptionsDictionary(broken_data, { parse: true });
...
test('dictionary.entries entrie_change event is triggered correctly', function () {
    ...
    var first_entry = dictionary.entries.at(0);

    var dictionary_event_counter = 0;
    var entry_event_counter = 0;

    dictionary.on('entries_change', function () { dictionary_event_counter += 1; });
    first_entry.on('change', function () { entry_event_counter += 1; });
