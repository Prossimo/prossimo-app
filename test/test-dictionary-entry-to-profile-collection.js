...
test('DictionaryEntryProfileCollection basic tests', function () {
    var dep_collection = new app.DictionaryEntryProfileCollection(null, { parse: true });

    ok(dep_collection instanceof Backbone.Collection, 'dep_collection is a Backbone.Collection object');
    equal(dep_collection.length, 0, 'dep_collection contains 0 entries by default');

    var deps_collection_with_data = new app.DictionaryEntryProfileCollection([
        {
            profile_id: 17,
            is_default: false
        },
    ...
    ok(
        deps_collection_with_data.at(0).get('pricing_grids') instanceof Backbone.Collection,
...
test('DictionaryEntryProfileCollection getByProfileId function', function () {
    var dep_collection = new app.DictionaryEntryProfileCollection([
        {
            profile_id: 17,
            is_default: false
        },
    ...
    var default_item = dep_collection.getByProfileId(22);
    var nondefault_item = dep_collection.getByProfileId(1);
    var nonexistent_item = dep_collection.getByProfileId(999);
...
test('DictionaryEntryProfileCollection parse function', function () {
    ...
    var first_item = dep_collection.at(0);
...
test('DictionaryEntryProfileCollection event propagation', function () {
    ...
    var dep = dep_collection.at(0);
    var fixed_grid_first_item = dep.get('pricing_grids').getByName('fixed').get('data').at(0);

    var dep_collection_event_counter = 0;
